...
function renderCategories(categories, activeCategory) {
    ...
    for (const cat of categories) {
        html += `<li class="category-item ${cat.name === activeCategory ? 'active' : ''}" data-category="${cat.name}">
...
function handleCombine(mode) {
    ...
    api.combine(selected, mode, title).then(note => {
...
function init() {
    ...
    if (combineConcatBtn) {
        combineConcatBtn.addEventListener('click', () => handleCombine('concatenate'));
    }
